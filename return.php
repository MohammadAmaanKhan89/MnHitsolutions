...
function render_result($status, $orderRef, $orderData, $extra = '') {
    ...
    echo '</div>';
    echo '<a class="btn" href="checkout.html">Back to MNH IT Solutions</a>';
...
try {
    ...
    // Bank Alfalah sometimes returns the JSON body double-encoded, i.e. a
    // JSON string whose *contents* are themselves JSON (e.g. "\"{...}\"").
    // In that case the first json_decode() just yields a plain PHP string,
    // not an array, so decode a second time to get the real payload.
    if (is_string($data)) {
